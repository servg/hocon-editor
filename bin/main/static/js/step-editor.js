...
const StepEditor = (function() {
    ...
    function renderStepEditorModal($body, draft, allSteps, typeDescriptor) {
        var step = draft.step;
        ...
        $body.html(
            ...
            (showConnection ? inputField("Connection", "step-connection-input", step.connection, "text", "oracle-main") : '<input type="hidden" id="step-connection-input" value="">') +
    ...
    function renderDynamicField(field, value) {
        ...
        if (field.type === 'textarea') {
            return '<div class="mb-2"><label class="form-label" for="' + id + '">' + esc(field.label) + req + '</label>' +
                '<textarea class="form-control form-control-sm config-api-field" id="' + id + '" data-key="' + esc(field.key) + '" rows="3" placeholder="' + esc(field.placeholder || '') + '">' + esc(value) + '</textarea>' +
                hint + '</div>';
        }
    ...
    function renderOutputTable(outputRows) {
        ...
        return '<table class="editor-table"><thead><tr><th>To</th><th>Condition</th><th></th></tr></thead>' +
            ...
            outputRows.map(function(row, i) {
                return '<tr><td><input class="form-control form-control-sm output-to" data-index="' + i + '" value="' + esc(row.to) + '" placeholder="next_step"></td>' +
                    ...
                    '<td><button type="button" class="btn btn-sm btn-outline-danger delete-output-row-btn" data-index="' + i + '">×</button></td></tr>';
            }).join('') +
            '</tbody></table>';
    }
